...
authRouter.post("/register", asyncHandler(async (req, res) => {
    ...
    return created(res, await auth.register(input), "Account created. Check your email to verify it.");
}));
authRouter.post("/login", asyncHandler(async (req, res) => {
    ...
    try {
        ...
        await recordPublicLogin({
            ...
            ...metadata,
        }).catch((auditError) => console.error("Could not write public login audit record", auditError));
        throw error;
    }
    await recordPublicLogin({
        email: body.email,
...
publicRouter.get("/events", asyncHandler(async (req, res) => {
    ...
    const result = await events.listEvents({
        ...
        status: status && Object.values(EventStatus).includes(status)
            ? status
            : undefined,
...
publicRouter.get("/events/:slug", asyncHandler(async (req, res) => ok(res, await events.getEventBySlug(param(req, "slug")))));
publicRouter.get("/event-categories", asyncHandler(async (_req, res) => ok(res, await prisma.eventCategory.findMany({ orderBy: { name: "asc" } }))));
publicRouter.get("/stats", asyncHandler(async (_req, res) => {
    const [eventsCount, usersCount, registrationsCount, certificatesCount] = await prisma.$transaction([
...
publicRouter.get("/certificates/verify/:certificateNumber", asyncHandler(async (req, res) => ok(res, await certificates.verifyCertificate(param(req, "certificateNumber")))));
router.use(publicRouter);
const userRouter = Router();
userRouter.use(requireAuth);
userRouter.get("/registrations/me", asyncHandler(async (req, res) => {
    const params = pageParams(req.query);
    const result = await registrations.listUserRegistrations(req.user.id, params.page, params.limit);
    return res.json({
        success: true,
        data: result.rows,
        pagination: pagination(params.page, params.limit, result.total),
...
userRouter.post("/events/:eventId/register", asyncHandler(async (req, res) => created(res, await registrations.registerForEvent(param(req, "eventId"), req.user.id, req.body?.answers), "Registration completed successfully")));
userRouter.get("/registrations/:id", asyncHandler(async (req, res) => ok(res, await registrations.getRegistration(param(req, "id"), req.user.id))));
userRouter.post("/registrations/:id/ticket", asyncHandler(async (req, res) => ok(res, await registrations.issueTicket(param(req, "id"), req.user.id), "Ticket issued")));
userRouter.patch("/registrations/:id/cancel", asyncHandler(async (req, res) => ok(res, await registrations.cancelRegistration(param(req, "id"), req.user.id), "Registration cancelled")));
userRouter.post("/events/:eventId/teams", asyncHandler(async (req, res) => {
...
userRouter.post("/teams/join", asyncHandler(async (req, res) => {
...
userRouter.get("/teams/:id", asyncHandler(async (req, res) => ok(res, await teams.getTeam(param(req, "id")))));
userRouter.post("/teams/:id/leave", asyncHandler(async (req, res) => {
    await teams.leaveTeam(param(req, "id"), req.user.id);
    return ok(res, null, "Left team");
}));
userRouter.post("/proofs/upload", upload.single("file"), asyncHandler(async (req, res) => {
    if (!req.file)
        throw new AppError("A proof file is required", 422, "FILE_REQUIRED");
    const body = z
        .object({
        ...
        .parse(req.body);
    return created(res, await proofs.uploadProof({
        ...body,
        userId: req.user.id,
...
userRouter.get("/proofs/:id", asyncHandler(async (req, res) => ok(res, await proofs.getProof(param(req, "id"), req.user.id))));
userRouter.delete("/proofs/:id", asyncHandler(async (req, res) => {
...
userRouter.get("/certificates/me", asyncHandler(async (req, res) => ok(res, await certificates.listMyCertificates(req.user.id))));
userRouter.get("/notifications", asyncHandler(async (req, res) => {
    const params = pageParams(req.query);
...
userRouter.patch("/notifications/:id/read", asyncHandler(async (req, res) => {
...
userRouter.patch("/notifications/read-all", asyncHandler(async (req, res) => {
    await prisma.notificationRecipient.updateMany({
...
userRouter.get("/profile", asyncHandler(async (req, res) => ok(res, await prisma.user.findUnique({
...
userRouter.patch("/profile", asyncHandler(async (req, res) => {
    const body = z
        ...
        .parse(req.body);
    return ok(res, await prisma.user.update({ where: { id: req.user.id }, data: body }), "Profile updated");
}));
router.use(userRouter);
const adminRouter = Router();
adminRouter.use(requireAuth);
adminRouter.get("/events", requirePermission("REGISTRATION_VIEW"), asyncHandler(async (req, res) => {
    const params = pageParams(req.query);
    const where = {
        deletedAt: null,
...
adminRouter.post("/events", requirePermission("EVENT_CREATE"), asyncHandler(async (req, res) => created(res, await events.createEvent(eventInput.parse(req.body), req.user.id), "Event created")));
adminRouter.get("/events/:id", requirePermission("REGISTRATION_VIEW"), asyncHandler(async (req, res) => ok(res, await events.getEventById(param(req, "id")))));
adminRouter.patch("/events/:id", requirePermission("EVENT_UPDATE"), asyncHandler(async (req, res) => {
...
adminRouter.post("/events/:id/publish", requirePermission("EVENT_PUBLISH"), asyncHandler(async (req, res) => {
    const id = param(req, "id");
...
adminRouter.post("/events/:id/cancel", requirePermission("EVENT_UPDATE"), asyncHandler(async (req, res) => {
    const id = param(req, "id");
...
adminRouter.post("/events/:id/archive", requirePermission("EVENT_UPDATE"), asyncHandler(async (req, res) => ok(res, await events.transitionEvent(param(req, "id"), "ARCHIVED"), "Event archived")));
adminRouter.get("/registrations", requirePermission("REGISTRATION_VIEW"), asyncHandler(async (req, res) => {
    const params = pageParams(req.query);
    const status = q(req.query.status);
    const result = await registrations.listAllRegistrations({
        ...params,
        eventId: q(req.query.eventId),
        status: status && Object.values(RegistrationStatus).includes(status)
            ? status
            : undefined,
        search: q(req.query.search),
    });
...
adminRouter.get("/events/:eventId/registrations", requirePermission("REGISTRATION_VIEW"), asyncHandler(async (req, res) => {
    const params = pageParams(req.query);
    const result = await registrations.listAllRegistrations({
        ...params,
...
adminRouter.patch("/registrations/:id/status", requirePermission("REGISTRATION_APPROVE"), asyncHandler(async (req, res) => {
    const id = param(req, "id");
...
adminRouter.post("/attendance/validate-ticket", requirePermission("ATTENDANCE_SCAN"), asyncHandler(async (req, res) => {
    const body = z
...
adminRouter.post("/attendance/check-in", requirePermission("ATTENDANCE_SCAN"), asyncHandler(async (req, res) => {
    const body = z
        .object({
...
adminRouter.post("/attendance/check-out", requirePermission("ATTENDANCE_SCAN"), asyncHandler(async (req, res) => {
    const body = z
        .object({
        token: z.string().min(10),
        eventId: z.string().uuid().optional(),
...
adminRouter.patch("/attendance/:id", requirePermission("ATTENDANCE_EDIT"), asyncHandler(async (req, res) => {
    const body = z
        .object({
        ...
        .parse(req.body);
    return ok(res, await attendance.overrideAttendance(param(req, "id"), req.user.id, body), "Attendance overridden");
}));
adminRouter.get("/events/:eventId/attendance", requirePermission("REGISTRATION_VIEW"), asyncHandler(async (req, res) => ok(res, await attendance.listEventAttendance(param(req, "eventId")))));
adminRouter.get("/proofs", requirePermission("PROOF_REVIEW"), asyncHandler(async (req, res) => {
    const params = pageParams(req.query);
    const status = q(req.query.status);
    const result = await proofs.listProofs({
        ...params,
        eventId: q(req.query.eventId),
        status: status && Object.values(ProofStatus).includes(status)
            ? status
            : undefined,
    });
    return res.json({
...
adminRouter.get("/proofs/:id", requirePermission("PROOF_REVIEW"), asyncHandler(async (req, res) => ok(res, await proofs.getProof(param(req, "id")))));
adminRouter.post("/proofs/:id/verify", requirePermission("PROOF_REVIEW"), asyncHandler(async (req, res) => ok(res, await proofs.reviewProof(param(req, "id"), req.user.id, "VERIFIED", req.body?.metadata ?? { reviewer: "manual" }, req), "Proof verified")));
adminRouter.post("/proofs/:id/reject", requirePermission("PROOF_REVIEW"), asyncHandler(async (req, res) => ok(res, await proofs.reviewProof(param(req, "id"), req.user.id, "REJECTED", req.body?.metadata ?? { reviewer: "manual" }, req), "Proof rejected")));
adminRouter.post("/proofs/:id/flag", requirePermission("PROOF_REVIEW"), asyncHandler(async (req, res) => ok(res, await proofs.reviewProof(param(req, "id"), req.user.id, "FLAGGED", req.body?.metadata ?? { reviewer: "manual" }, req), "Proof flagged")));
adminRouter.post("/events/:eventId/certificates/generate", requirePermission("CERTIFICATE_GENERATE"), asyncHandler(async (req, res) => ok(res, await certificates.generateCertificates(param(req, "eventId"), req.user.id, req), "Certificates generated")));
adminRouter.post("/certificates/:id/revoke", requirePermission("CERTIFICATE_GENERATE"), asyncHandler(async (req, res) => ok(res, await certificates.revokeCertificate(param(req, "id"), req), "Certificate revoked")));
adminRouter.get("/analytics/overview", requirePermission("REPORT_EXPORT"), asyncHandler(async (_req, res) => {
    const [totalEvents, upcomingEvents, activeEvents, completedEvents, totalUsers, registrations, pendingProofs, checkins,] = await prisma.$transaction([
        prisma.event.count({ where: { deletedAt: null } }),
...
adminRouter.get("/audit-logs", requirePermission("AUDIT_LOG_VIEW"), asyncHandler(async (req, res) => {
    const params = pageParams(req.query);
    const [total, rows] = await prisma.$transaction([
...
adminRouter.get("/reports/registrations", requirePermission("REPORT_EXPORT"), asyncHandler(async (req, res) => {
    const result = await registrations.listAllRegistrations({
        page: 1,
        limit: 10000,
        eventId: q(req.query.eventId),
...
adminRouter.get("/users", requirePermission("USER_MANAGE"), asyncHandler(async (req, res) => {
    const params = pageParams(req.query);
    const where = {
        deletedAt: null,
        ...(q(req.query.search)
...
adminRouter.patch("/users/:id/status", requirePermission("USER_MANAGE"), asyncHandler(async (req, res) => {
    const id = param(req, "id");
    ...
    await audit(req, body.isActive ? "USER_ACTIVATED" : "USER_DEACTIVATED", "User", id, old, updated);
