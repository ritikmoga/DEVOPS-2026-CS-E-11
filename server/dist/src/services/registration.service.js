...
export async function registerForEvent(eventId, userId, answers) {
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (existing && existing.status !== "CANCELLED")
            throw new AppError("You are already registered for this event", 409, "DUPLICATE_REGISTRATION");
        let hasSeat = false;
    ...
    if (result.status === "CONFIRMED")
        await notifyUsers([userId], "REGISTRATION_CONFIRMED", "Registration confirmed", `Your registration ${result.registrationNumber} is confirmed.`);
    return result;
}
...
export async function issueTicket(id, userId) {
    ...
    if (!isConfirmed(registration.status))
        throw new AppError("A ticket is available only for a confirmed registration", 409, "TICKET_NOT_AVAILABLE");
...
export async function promoteWaitlisted(tx, eventId) {
    ...
    await notifyUsers([candidate.userId], "WAITLIST_PROMOTED", "You are off the waitlist", `Registration ${candidate.registrationNumber} is now confirmed.`);
}
