...
export async function createApplication(input) {
  ...
  if (existing)
    throw new AppError(
      "You already have an OD application for this event",
      409,
      "DUPLICATE_OD_APPLICATION",
    );
...
export async function submitApplication(id, requesterId, req) {
  ...
  if (!["DRAFT", "REJECTED"].includes(application.status))
    throw new AppError(
      "This OD application cannot be submitted in its current state",
      409,
      "INVALID_OD_STATE",
    );
...
export async function decideApplication(id, actorId, level, decision, comments) {
  ...
  if (!application) throw new AppError("OD application not found", 404, "OD_NOT_FOUND");
  const allowed = {
  ...
  if (!allowed[level].includes(application.status))
    throw new AppError(
      `This application is not ready for ${level.toLowerCase()} review`,
      409,
      "INVALID_OD_APPROVAL_STAGE",
    );
  const nextStatus =
    decision === "REJECTED"
      ? "REJECTED"
    ...
    const next = await tx.oDApplication.update({
      ...
      data: {
        ...
        currentApprovalLevel:
          decision === "APPROVED" && level === "FACULTY"
            ? "HOD"
...
export async function submitCompletion(input, req) {
  ...
  if (!["COMPLETION_PENDING", "EVENT_COMPLETED", "FLAGGED"].includes(application.status))
    throw new AppError(
      "Completion cannot be submitted at this stage",
      409,
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    for (const member of members) {
      const attendance = registrations.find(
        (registration) => registration.userId === member.userId,
      )?.attendance;
      const memberProof = proofs.some(
        (proof) => proof.userId === member.userId && proof.verificationStatus === "VERIFIED",
      );
      ...
      if (finalStatus === "VERIFIED_ATTENDED") verified++;
  ...
  await audit(
    req,
    "OD_COMPLETION_SUBMITTED",
    "ODApplication",
    input.odApplicationId,
    application,
    result.application,
  );
  return result;
