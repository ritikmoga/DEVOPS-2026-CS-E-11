...
function validateLocation(event, location) {
    if (event.latitude == null || event.longitude == null || !event.allowedRadiusMeters)
        return;
    if (location?.latitude == null || location.longitude == null)
        throw new AppError("Location is required for this event", 422, "LOCATION_REQUIRED");
    const distance = distanceMeters(Number(event.latitude), Number(event.longitude), location.latitude, location.longitude);
    if (distance > event.allowedRadiusMeters)
        throw new AppError("You are outside the permitted attendance area", 403, "OUTSIDE_ATTENDANCE_RADIUS", [{ distanceMeters: Math.round(distance), allowedRadiusMeters: event.allowedRadiusMeters }]);
}
async function findTicket(token, eventId) {
    ...
    if (!["CONFIRMED", "CHECKED_IN", "CHECKED_OUT"].includes(ticket.registration.status))
        throw new AppError("Registration is not eligible for attendance", 409, "REGISTRATION_NOT_ELIGIBLE");
    return ticket;
}
...
export async function checkOut(token, actorId, eventId, location) {
    ...
    const totalEventMinutes = Math.max(1, Math.round((event.endAt.getTime() - event.startAt.getTime()) / 60000));
