...
async function loadRoles(userId) {
    ...
    return {
        ...
        permissions: [
            ...new Set(roles.flatMap((item) => item.role.permissions.map((entry) => entry.permission.code))),
        ],
    };
}
export async function register(input) {
    ...
    if (existing)
        throw new AppError("An account with this email or enrollment number already exists", 409, "ACCOUNT_EXISTS");
    ...
    await sendEmail(user.email, "Verify your EventHub account", `<p>Hello ${user.fullName}, verify your account using token <strong>${verificationToken}</strong>.</p>`);
    return user;
}
export async function login(email, password, metadata) {
    ...
    const accessToken = jwt.sign({ sub: user.id, sessionId: session.id, type: "access" }, env.JWT_ACCESS_SECRET, { expiresIn: env.ACCESS_TOKEN_EXPIRES_IN });
...
export async function refresh(refreshToken, metadata) {
    ...
    const accessToken = jwt.sign({ sub: session.userId, sessionId: nextSession.id, type: "access" }, env.JWT_ACCESS_SECRET, { expiresIn: env.ACCESS_TOKEN_EXPIRES_IN });
...
export async function requestPasswordReset(email) {
    ...
    await sendEmail(user.email, "Reset your EventHub password", `<p>Use this password reset token: <strong>${token}</strong></p>`);
}
